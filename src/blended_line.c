...
void blended_line(const float start_opacity, const float start_red,
                  const float start_green, const float start_blue,
                  const float start_row, const float start_column,
                  const float start_depth, const float end_opacity,
                  const float end_red, const float end_green,
                  const float end_blue, const float end_row,
                  const float end_column, const float end_depth,
                  const int viewport_rows, const int viewport_columns,
  ...
  const int rounded_start_row =
      start_row < 0 ? ((int)start_row) - 1 : ((int)start_row);
  const int rounded_start_column =
      start_column < 0 ? ((int)start_column) - 1 : ((int)start_column);

  const int rounded_end_row = end_row < 0 ? ((int)end_row) - 1 : ((int)end_row);
  const int rounded_end_column =
      end_column < 0 ? ((int)end_column) - 1 : ((int)end_column);
  ...
  const int absolute_column_delta =
      column_delta >= 0.0f ? column_delta : -column_delta;

  int start_primary_axis, end_primary_axis, indices_per_primary_axis,
      maximum_primary_axis, indices_per_secondary_axis, maximum_secondary_axis;
  ...
  if (end_primary_axis > start_primary_axis) {
    ...
    accumulators = starts;
    progress_per_pixel =
        1.0f / ((float)(end_primary_axis + 1 - start_primary_axis));
  } else {
  ...
  last_primary_axis = last_primary_axis < maximum_primary_axis
                          ? last_primary_axis
                          : maximum_primary_axis;

  for (int primary_axis = first_primary_axis; primary_axis <= last_primary_axis;
       primary_axis++) {
    const int secondary_axis = accumulators[0];

    if (secondary_axis >= 0 && secondary_axis < maximum_secondary_axis) {
      const int index = primary_axis * indices_per_primary_axis +
